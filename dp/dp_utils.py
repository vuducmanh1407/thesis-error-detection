...
def compute_all_costs(
    ...
    if drop_cost_type == "logit":
        ...
        if given_baseline_logits is not None:
            baseline_logits = given_baseline_logits
        else:
    ...
    if do_logsoftmax:
        ...
        zx_costs = -sim
        drop_costs = -baseline_logits[0]
        drop_probs = None

    if return_baseline:
        return zx_costs, drop_costs, drop_probs, baseline_logits
    else:
    ...
    def transfer_device(self, device):
        for i in range(self.d1):
            for j in range(self.d2):
                self.vars[i][j] = self.vars[i][j].to(device)
...
def minProb(inputs, gamma=1, keepdim=True):
    ...
    if gamma == 0:
        ...
        minP, _ = torch.min(inputs, dim=0, keepdim=keepdim)
    else:
        #probs = F.softmax(-inputs / gamma, dim=0)
        # save computation, ignore gamma
        probs = F.softmax(-inputs, dim=0)
        minP = (probs * inputs).sum(dim=0, keepdim=keepdim)
    return minP

def real_minProb(inputs, gamma=1):
    if type(inputs) == list:
        if inputs[0].shape[0] == 1:
            inputs = torch.cat(inputs)
        else:
            inputs = torch.stack(inputs, dim=0)
    minP, _ = torch.min(inputs, dim=0, keepdim=True)
    return minP
