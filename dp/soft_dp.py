...
from dp.dp_utils import VarTable, minGamma, minProb, real_minProb
...
def generalized_metadag2vid_soft(zx_costs, drop_costs, node_drop_costs, metadag, idx2node, softning="minGamma", gamma_min=1):
    """a soft (differentiable) version of generalized graph2vid
    ...
    return_label: bool
        if True, returns output directly useful for segmentation computation (made for convenience)
    """
    # zx_costs = zx_costs.to(device)
    # drop_costs = drop_costs.to(device)
    # node_drop_costs = node_drop_costs.to(device)

    device = zx_costs.get_device()

    zx_costs = zx_costs.cpu()
    drop_costs = drop_costs.cpu()
    node_drop_costs = node_drop_costs.cpu()
    ...
    #     zx_costs = torch.stack([zx_costs] * mult, dim=-1).reshape([K, -1])
    #     drop_costs = torch.stack([drop_costs] * mult, dim=-1).reshape([-1])
    #     node_drop_costs = torch.stack([node_drop_costs] * mult, dim=-1).reshape([-1])
    #     N *= mult

    cum_drop_costs = torch.cumsum(drop_costs, dim=0)
    cum_node_drop_costs = torch.cumsum(node_drop_costs, dim=0)

    # Creating and initializing DP tables
    # the M + 2 last dimensions correspond to different states.
    # M types + 2 drops
    D = VarTable((K + 1, N + 1, M + 2), device=zx_costs.device)

    for zi in range(1, K + 1):
        # D[zi, 0] = torch.zeros_like(D[zi, 0]) + inf
        D[zi, 0] = torch.zeros_like(D[zi, 0]) + cum_node_drop_costs[(zi - 1):zi]
    for xi in range(1, N + 1):
        D[0, xi] = torch.zeros_like(D[0, xi]) + cum_drop_costs[(xi - 1) : xi]
    ...
    for zi in range(1, K + 1):
        for xi in range(1, N + 1):
            ...
            for pz in prev_states_dict[zi]:
                # min_val = min_fn([D[pz, xi - 1][[0]], D[pz, xi - 1][[1]]], gamma=gamma_min)
                # min_val = min_fn([D[pz, xi - 1][[s]] for s in range(M + 2)], gamma=gamma_min)

                # hard coding to speed up training
                min_val = min_fn([D[pz, xi - 1][[0]],
                                  D[pz, xi - 1][[1]],
                                  D[pz, xi - 1][[2]],
                                  D[pz, xi - 1][[3]],
                                  D[pz, xi - 1][[4]],
                                  D[pz, xi - 1][[5]]], gamma=gamma_min)
                prev_costs.append(min_val)

            # cur_costs = [D[zi, xi - 1][[s]] for s in [0, 1]]
            cur_costs = [D[zi, xi - 1][[s]] for s in range(M + 2)]
            # cur_pos_costs = cur_costs[:1] if contiguous else cur_costs
            cur_pos_costs = [cur_costs[s] for s in pos_states]

            z_cost_ind, x_cost_ind = zi - 1, xi - 1  # indexind in costs is shifted by 1

            # positive transition, i.e., matching step (either normal or erroneous step)
            pi = 0
            Dp = []
            # consider everything together
            # all_pos_costs = prev_costs + cur_pos_costs
            # min_cur_cost = min_fn(all_pos_costs, gamma=gamma_min)

            # first consider current mathcing, and the min with previous costs, v3
            min_cur_pos_cost = min_fn(cur_pos_costs, gamma=gamma_min)
            all_pos_costs = prev_costs + [min_cur_pos_cost]
            min_prev_cur_cost = min_fn(all_pos_costs, gamma=gamma_min)
            for ps in pos_states:
                Dp.append(min_prev_cur_cost + zx_costs[pi][z_cost_ind, x_cost_ind])
                pi += 1

            # negative transition, i.e. dropping xi
            Dn_x = min_fn(cur_costs, gamma=gamma_min) + drop_costs[x_cost_ind]

            # negative transition, i.e., dropping node zi
            parent_costs = []
            for pz in prev_states_dict[zi]:
                # min_val = min_fn([D[pz, xi][[s]] for s in range(M + 2)], gamma=gamma_min)

                # hard coding to speed up training
                min_val = min_fn([D[pz, xi][[0]],
                                  D[pz, xi][[1]],
                                  D[pz, xi][[2]],
                                  D[pz, xi][[3]],
                                  D[pz, xi][[4]],
                                  D[pz, xi][[5]]], gamma=gamma_min)
                parent_costs.append(min_val)

            Dn_z = min_fn(parent_costs, gamma=gamma_min) + node_drop_costs[z_cost_ind]

            # update final solution matrix
            final_D = [Dp[0], Dn_x, Dn_z]
            for i in range(1, len(pos_states)):
                final_D.append(Dp[i])
            D[zi, xi] = torch.cat(final_D, dim=0)

    # Computing the final min cost for the whole batch
    # min_cost = min_fn([D[K, N][[0]], D[K, N][[1]]], gamma=gamma_min)

    # transfer to gpu
    # D.transfer_device(device)
    # zx_costs = zx_costs.to(device)
    # drop_costs = drop_costs.to(device)
    # node_drop_costs = node_drop_costs.to(device)

    min_cost = min_fn([D[K, N][[s]] for s in range(M + 2)], gamma=gamma_min)
    # return min_cost, D
    return min_cost.to(device), D
